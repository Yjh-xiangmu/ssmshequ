...
<html lang="zh-CN">
<head>
<meta charset="UTF-8"><title>病例管理 - 管理后台</title>
...
<div class="content">
    ...
    <div class="page-sub">病例数据全生命周期监管、质量审核与统计。</div>
    ...
    <div class="card">
        ...
        <table>
            <tr><th>患者</th><th>联系电话</th><th>主诊医生</th><th>就诊日期</th><th>主诉</th><th>诊断结果</th><th>状态</th><th>操作</th></tr>
            ...
            <tr>
                ...
                <td>
                    <button class="btn btn-edit btn-sm" onclick="openEdit(${c.id},${c.userId},${c.doctorId},'<fmt:formatDate value="${c.visitDate}" pattern="yyyy-MM-dd"/>','${c.chiefComplaint}','${c.diagnosis}','${c.prescription}','${c.remark}',${c.status})">编辑</button>
                    <a href="/admin/case/delete?id=${c.id}" class="btn btn-danger btn-sm" onclick="return confirm('确认删除？')">删除</a>
                </td>
            </tr>
            </c:forEach>
        </table>
...
<div class="modal">
    <h3>新增病例</h3>
    <form action="/admin/case/add" method="post">
        <div class="form-row">
            ...
            <div class="form-group"><label>主诊医生</label>
                <select name="doctorId">
                    <c:forEach var="d" items="${doctors}"><option value="${d.id}">${d.name}（${d.department}）</option></c:forEach>
                </select>
            </div>
        </div>
        <div class="form-group"><label>就诊日期</label><input name="visitDate" type="date" required></div>
        <div class="form-group"><label>主诉</label><textarea name="chiefComplaint" rows="2"></textarea></div>
        <div class="form-group"><label>诊断结果</label><textarea name="diagnosis" rows="2"></textarea></div>
...
<div class="modal">
    <h3>编辑病例</h3>
    <form action="/admin/case/edit" method="post">
        ...
        <div class="form-row">
            ...
            <div class="form-group"><label>主诊医生</label>
                <select name="doctorId" id="eDoctorId">
                    <c:forEach var="d" items="${doctors}"><option value="${d.id}">${d.name}（${d.department}）</option></c:forEach>
                </select>
            </div>
        </div>
...
function openEdit(id,uid,did,vdate,complaint,diag,presc,remark,status) {
    document.getElementById('eId').value=id;
    document.getElementById('eUserId').value=uid;
    document.getElementById('eDoctorId').value=did;
